...
function ExternalButton({ href, children, secondary = false }: { href: string; children: string; secondary?: boolean }) {
  ...
  return href
    ? <a className={className} href={href} target="_blank" rel="noreferrer">{children}<span aria-hidden="true"> ↗</span></a>
...
function PartyCard({ member }: { member: WeddingPartyMember }) {
  return <article className="party-card">
    <div className="party-portrait" aria-hidden="true">{member.photo ? <img src={member.photo} alt="" /> : '✿'}</div>
...
function App() {
  return <div className="site-shell">
    ...
    <header className="site-header">
      <a className="monogram" href="#home" aria-label="Leah and Peter home">L<span>♥</span>P</a>
      <nav aria-label="Primary navigation">{navItems.map(([label, id]) => <a key={id} href={`#${id}`}>{label}</a>)}</nav>
    </header>
    <main>
      <section id="home" className="hero-section">
        <div className="hero-vine hero-vine-top" aria-hidden="true">✿ ── ❀ ── ♥ ── ❀ ── ✿</div>
      ...
      <section id="travel" className="section travel-section">
        ...
        <div className="travel-grid">
          <article className="info-card venue-card"><span className="card-icon" aria-hidden="true">⌂</span><p className="eyebrow">Our venue</p><h3>{wedding.venue.name}</h3><p>{wedding.venue.address}</p><p className="card-copy">Our ceremony and reception will both take place at Lauren Rose Farm. Parking details will be shared closer to the celebration.</p><ExternalButton href={wedding.venue.mapUrl} secondary>Get directions</ExternalButton></article>
          <article className="info-card"><span className="card-icon" aria-hidden="true">♥</span><p className="eyebrow">Official hotel block</p><h3>Residence Inn Columbus Easton</h3><p>Leah & Peter Wedding Block</p><ul><li>Complimentary parking and hot breakfast</li><li>Suite accommodations with full kitchens</li><li>At Easton Town Center, ideal for families</li></ul><p className="deadline">Book by May 7, 2027</p><ExternalButton href={wedding.links.hotel}>Book hotel</ExternalButton></article>
          <article className="info-card"><span className="card-icon" aria-hidden="true">✈</span><p className="eyebrow">Flying in?</p><h3>John Glenn Columbus International Airport</h3><p>CMH is the most convenient airport for our out-of-town guests.</p><p className="card-copy">While you’re nearby, explore Easton Town Center for restaurants, coffee, shopping, entertainment, and a nearby Trader Joe’s.</p></article>
        </div>
      </section>

      <section id="rsvp" className="cta-section"><div className="cta-flower" aria-hidden="true">✿</div><p className="script-accent">Please join us</p><h2>RSVP</h2><p>We cannot wait to celebrate with you! Please click below to RSVP through The Knot.</p><ExternalButton href={wedding.links.rsvp}>RSVP here</ExternalButton></section>
      ...
      <section id="story" className="section story-section">
        <div className="story-illustration" aria-hidden="true"><span>♥</span><span>✿</span><span>❀</span></div>
